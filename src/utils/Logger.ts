...
if (Configuration.getLogRotate()) {
  ...
  transports = [
    new DailyRotateFile({
      filename: Utils.insertAt(Configuration.getLogErrorFile(), '-%DATE%', Configuration.getLogErrorFile().indexOf('.log')),
      level: 'error',
  ...
  transports = [new File({ filename: Configuration.getLogErrorFile(), level: 'error' }), new File({ filename: Configuration.getLogFile() })];
}
