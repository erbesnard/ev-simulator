...
export default class ChargingStation {
  ...
  public isRegistered(): boolean {
    return (
      this.bootNotificationResponse?.status === RegistrationStatus.ACCEPTED
    );
  }
  ...
  public getTransactionIdTag(transactionId: number): string | undefined {
    for (const connector in this.connectors) {
      if (
        ...
        this.getConnector(Utils.convertToInt(connector)).transactionId ===
          transactionId
      ) {
  ...
  public getLocalAuthListEnabled(): boolean {
    ...
    return localAuthListEnabled
      ? Utils.convertToBoolean(localAuthListEnabled.value)
      : false;
  }
  ...
  public getSampledValueTemplate(
    ...
    const sampledValueTemplates: SampledValueTemplate[] = this.getConnector(
      connectorId
    ).MeterValues;
    for (
      let index = 0;
      !Utils.isEmptyArray(sampledValueTemplates) &&
      index < sampledValueTemplates.length;
      index++
    ) {
      if (
  ...
  public getAutomaticTransactionGeneratorRequireAuthorize(): boolean {
    return (
      this.stationInfo.AutomaticTransactionGenerator.requireAuthorize ?? true
    );
  }
  ...
  private buildStationInfo(): ChargingStationInfo {
    ...
    const stationInfo: ChargingStationInfo =
      stationTemplateFromFile ?? ({} as ChargingStationInfo);
    ...
    stationInfo.chargingStationId = this.getChargingStationId(
  ...
  private getOCPPVersion(): OCPPVersion {
    return this.stationInfo.ocppVersion
      ? this.stationInfo.ocppVersion
  ...
  private initialize(): void {
    ...
    const connectorsConfigHash = crypto
      .createHash("sha256")
      .update(
        JSON.stringify(this.stationInfo.Connectors) + maxConnectors.toString()
      )
      .digest("hex");
    ...
    if (
      ...
      for (lastConnector in this.stationInfo.Connectors) {
        if (
          ...
          this.connectors[lastConnector].availability =
            AvailabilityType.OPERATIVE;
          if (
            Utils.isUndefined(this.connectors[lastConnector]?.chargingProfiles)
          ) {
    ...
    for (const connector in this.connectors) {
      if (
        ...
        this.initTransactionAttributesOnConnector(
          Utils.convertToInt(connector)
        );
      }
    }
    switch (this.getOCPPVersion()) {
      case OCPPVersion.VERSION_16:
        this.ocppIncomingRequestService = new OCPP16IncomingRequestService(
          this
        );
        this.ocppRequestService = new OCPP16RequestService(
          this,
          new OCPP16ResponseService(this)
        );
        break;
      default:
  ...
  private initOCPPParameters(): void {
    if (
      !this.getConfigurationKey(StandardParametersKey.SupportedFeatureProfiles)
    ) {
    ...
    if (
      !this.getConfigurationKey(StandardParametersKey.MeterValuesSampledData)
    ) {
      this.addConfigurationKey(
        StandardParametersKey.MeterValuesSampledData,
        MeterValueMeasurand.ENERGY_ACTIVE_IMPORT_REGISTER
    ...
    if (
      !this.getConfigurationKey(StandardParametersKey.ConnectorPhaseRotation)
    ) {
      ...
      for (const connector in this.connectors) {
        ...
        if (
          Utils.convertToInt(connector) === 0 &&
          this.getNumberOfPhases() === 0
        ) {
          connectorPhaseRotation.push(
            `${connector}.${ConnectorPhaseRotation.RST}`
          );
        } else if (
          Utils.convertToInt(connector) > 0 &&
          this.getNumberOfPhases() === 0
          ...
          connectorPhaseRotation.push(
            `${connector}.${ConnectorPhaseRotation.NotApplicable}`
          );
          // AC
        } else if (
          Utils.convertToInt(connector) > 0 &&
          this.getNumberOfPhases() === 1
        ) {
          connectorPhaseRotation.push(
            `${connector}.${ConnectorPhaseRotation.NotApplicable}`
          );
    ...
    if (
      !this.getConfigurationKey(StandardParametersKey.AuthorizeRemoteTxRequests)
    ) {
      this.addConfigurationKey(
        StandardParametersKey.AuthorizeRemoteTxRequests,
        "true"
    ...
    if (
      ...
      this.getConfigurationKey(
        StandardParametersKey.SupportedFeatureProfiles
      ).value.includes(SupportedFeatureProfiles.Local_Auth_List_Management)
    ) {
      this.addConfigurationKey(
        StandardParametersKey.LocalAuthListEnabled,
        "false"
  ...
  private async onMessage(data: Data): Promise<void> {
    let [
      ...
      commandPayload,
      errorDetails
    ]: IncomingRequest = [0, "", "" as IncomingRequestCommand, {}, {}];
    ...
    try {
      ...
      switch (messageType) {
        ...
        case MessageType.CALL_MESSAGE:
          if (this.getEnableStatistics()) {
            this.performanceStatistics.addRequestStatistic(
              commandName,
              messageType
            );
          }
  ...
  private onPing(): void {
    logger.debug(
      this.logPrefix() + " Received a WS ping (rfc6455) from the server"
    );
  }

  private onPong(): void {
    logger.debug(
      this.logPrefix() + " Received a WS pong (rfc6455) from the server"
    );
  }
  ...
  private getTemplateChargingStationConfiguration(): ChargingStationConfiguration {
    return (
      this.stationInfo.Configuration ?? ({} as ChargingStationConfiguration)
    );
  }
  ...
  private getConnectionTimeout(): number | undefined {
    if (this.getConfigurationKey(StandardParametersKey.ConnectionTimeOut)) {
      return (
        parseInt(
          this.getConfigurationKey(StandardParametersKey.ConnectionTimeOut)
            .value
  ...
  private getMaxNumberOfConnectors(): number {
    ...
    if (!Utils.isEmptyArray(this.stationInfo.numberOfConnectors)) {
      const numberOfConnectors = this.stationInfo
        .numberOfConnectors as number[];
      // Distribute evenly the number of connectors
      maxConnectors =
        numberOfConnectors[(this.index - 1) % numberOfConnectors.length];
  ...
  private async startMessageSequence(): Promise<void> {
    ...
    for (const connector in this.connectors) {
      if (Utils.convertToInt(connector) === 0) {
        ...
        this.getConnector(
          Utils.convertToInt(connector)
        ).status = this.getConnector(Utils.convertToInt(connector)).bootStatus;
      } else if (
        this.stopped &&
        this.getConnector(Utils.convertToInt(connector))?.bootStatus
      ) {
  ...
  private startAutomaticTransactionGenerator() {
    if (this.stationInfo.AutomaticTransactionGenerator.enable) {
      if (!this.automaticTransactionGenerator) {
        this.automaticTransactionGenerator = new AutomaticTransactionGenerator(
          this
        );
      }
    ...
    if (
      ...
      for (const connector in this.connectors) {
        if (
          ...
          const transactionId = this.getConnector(Utils.convertToInt(connector))
            .transactionId;
  ...
  private startWebSocketPing(): void {
    const webSocketPingInterval: number = this.getConfigurationKey(
      ...
      ? Utils.convertToInt(
          this.getConfigurationKey(StandardParametersKey.WebSocketPingInterval)
            .value
        )
      : 0;
  ...
  private getHeartbeatInterval(): number | undefined {
    const HeartbeatInterval = this.getConfigurationKey(
  ...
  private openWSConnection(
    ...
    logger.info(
      this.logPrefix() +
        " Open OCPP connection to URL " +
        this.wsConnectionUrl.toString()
    );
  }
        ...
        fs.watch(authorizationFile, (event, filename) => {
          if (filename && event === "change") {
            try {
              ...
              logger.error(
                this.logPrefix() + " Authorization file monitoring error: %j",
                error
              );
            }
          }
  ...
  private startStationTemplateFileMonitoring(): void {
    try {
      fs.watch(
        this.stationTemplateFile,
        async (event, filename): Promise<void> => {
          if (filename && event === "change") {
            try {
              logger.debug(
                this.logPrefix() +
              ...
              logger.error(
                this.logPrefix() +
                  " Charging station template file monitoring error: %j",
                error
              );
            }
          }
