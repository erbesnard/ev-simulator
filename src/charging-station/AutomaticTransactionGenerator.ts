...
export default class AutomaticTransactionGenerator {
  ...
  public start(): void {
    const previousRunDuration =
      this?.startDate && this?.lastRunDate
    ...
    for (const connector in this.chargingStation.connectors) {
      if (Utils.convertToInt(connector) > 0) {
        ...
        setImmediate(() => {
          this.startOnConnector(Utils.convertToInt(connector)).catch(() => {
            /* This is intentional */
          });
        });
      }
    ...
    logger.info(
      this.logPrefix() +
        " started and will run for " +
        Utils.formatDurationMilliSeconds(
          this.stopDate.getTime() - this.startDate.getTime()
        )
    );
  }
  ...
  public stop(): void {
    ...
    this.started = false;
    logger.info(
      `${this.logPrefix()} over and lasted for ${Utils.formatDurationMilliSeconds(
        this.lastRunDate.getTime() - this.startDate.getTime()
    ...
    while (this.started) {
      ...
      if (!this.chargingStation.isConnectorAvailable(connectorId)) {
        logger.info(
          `${this.logPrefix(
            connectorId
          )} Entered in transaction loop while the connector ${connectorId} is unavailable, stop it`
        );
        break;
      }
      ...
      logger.info(
        this.logPrefix(connectorId) +
          " waiting for " +
          Utils.formatDurationMilliSeconds(wait)
      );
      ...
      if (
        ...
        logger.info(
          this.logPrefix(connectorId) +
            " skipped transaction " +
            skippedTransactions.toString() +
            "/" +
            skippedTransactionsTotal.toString()
        );
      }
  ...
  private async startTransaction(
    ...
    if (this.chargingStation.hasAuthorizedTags()) {
      ...
      if (
        this.chargingStation.getAutomaticTransactionGeneratorRequireAuthorize()
      ) {
        // Authorize tagId
      ...
      logger.info(
        this.logPrefix(connectorId) + " start transaction for tagID " + tagId
      );
      // Start transaction
      startResponse = await this.chargingStation.ocppRequestService.sendStartTransaction(
        connectorId,
    ...
    logger.info(
      this.logPrefix(connectorId) + " start transaction without a tagID"
    );
    startResponse = await this.chargingStation.ocppRequestService.sendStartTransaction(
      connectorId
    );
  ...
  private async stopTransaction(
    ...
    if (this.chargingStation.getConnector(connectorId)?.transactionStarted) {
      transactionId = this.chargingStation.getConnector(connectorId)
        .transactionId;
      ...
      logger.warn(
        `${this.logPrefix(
          connectorId
        )} trying to stop a not started transaction${
